...
export default function CollegePredictorV2() {
  ...
  const fetchPredictions = async () => {
    ...
    try {
      ...
      console.log("V2 Prediction response:", response.data);
      setResults(response.data);
    } catch (error) {
      console.error("V2 Prediction error:", error);
              ...
              <div className="flex space-x-1.5 sm:space-x-2">
                {options.genders.map((option) => (
                  <button
                    ...
                    className={`flex-1 p-2 sm:p-3 border rounded-lg text-xs sm:text-sm font-medium transition ${formData.gender === option.value
