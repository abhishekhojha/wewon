  ...
  return {
    ...
    category:
      prefill.category && (!preserveExistingValues || !prev.category)
        ? isJACDelhi && prefill.category === "GEN"
          ? "OPEN"
          : prefill.category
        : prev.category,
    homeState:
    ...
    districts:
      prefill.districts &&
      (!preserveExistingValues || prev.districts.length === 0)
        ? prefill.districts
        : prev.districts,
  };
};
...
export default function ChoiceFillingForm({
  ...
  const {
    ...
    jeeAdvanceAccess,
  } = useJeeAdvancedGates(isAuthenticated, userOrders);
  ...
  const [isDevelopmentMode, setIsDevelopmentMode] = useState(
    process.env.NODE_ENV === "development" ||
      process.env.NEXT_PUBLIC_API_URL ===
        "https://wewon-backend-dev.vercel.app/",
  );
  const isChoiceFillingLocked = isIIT
    ? iitChoiceFillingLocked
    : isOrderChoiceFillingLocked;
  ...
  const [submittedToolKey, setSubmittedToolKey] = useState<string>(
    toolKey || "jee-main",
  );
  ...
  const isCrlRankRequired =
    isJACDelhi || !isIIT || formData.category === "OPEN";
  const isCategoryRankRequired =
    !isUPTAC && !isJACDelhi && !isCSAB && formData.category !== "OPEN";
  ...
  useEffect(() => {
    if (orderPrefill) {
      if (toolKey === "uptac" && (orderPrefill as any).districts) {
        (orderPrefill as any).districts = (
          orderPrefill as any
        ).districts.filter((d: string) => d.toLowerCase() !== "kaushambi");
      }
      const mergedPrefill = {
        ...orderPrefill,
        ...(isIIT &&
        jeeAdvanceAccess &&
        typeof jeeAdvanceAccess.jeeAdvancedRank === "number"
          ? { jeeAdvancedRank: jeeAdvanceAccess.jeeAdvancedRank }
          : {}),
      };
      setFormData((prev) =>
        mergePrefillIntoForm(prev, mergedPrefill, false, isJACDelhi, isIIT),
      );
    }

    const hasPrefilledCrl = isIIT
      ? isValidRank(orderPrefill?.jeeAdvancedRank) ||
        isValidRank(jeeAdvanceAccess?.jeeAdvancedRank)
    ...
    const loadMetadata = async () => {
      try {
        ...
        if (toolKey === "uptac" && data) {
          if (data.districts) {
            data.districts = data.districts.filter(
              (d: string) => d.toLowerCase() !== "kaushambi",
            );
          }
          if (data.prefill?.districts) {
            data.prefill.districts = data.prefill.districts.filter(
              (d: string) => d.toLowerCase() !== "kaushambi",
            );
          }
        }
        ...
        if (prefill) {
          ...
          setFormData((prev) =>
            mergePrefillIntoForm(prev, mergedPrefill, true, isJACDelhi, isIIT),
          );
        }

        const isMetadataCrlPrefilled = isIIT
          ? isValidRank(prefill?.jeeAdvancedRank) ||
            isValidRank(jeeAdvanceAccess?.jeeAdvancedRank)
        ...
        setRankLocked(
          Boolean(
            (data.rankLocked || isOrderRankLocked || isMetadataCrlPrefilled) &&
            hasPrefilledCrl &&
            !isDevelopmentMode &&
            !isIIT,
          ),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (
      isJACDelhi &&
      isGirlRelated(formData.subCategory) &&
    ...
    try {
      const payload: ChoiceFillingRequest = {
        ...
        categoryRank: formData.categoryRank
          ? formData.categoryRank.includes("P")
      ...
      const finalBranchGroups =
        formData.branchGroups.length > 0 &&
        formData.branchGroups.length < availableBranchGroups.length
      ...
      if (isJACDelhi) {
        ...
        payload.includedIITs = (() => {
          ...
          const resolved = formData.includedIITs.map(
            (sn) => iitMap.get(sn) ?? sn,
          );
        ...
        if (isUPTAC) {
          ...
          payload.districts =
            ...
            formData.districts.length < allDistricts.length
              ? formData.districts
              : undefined;
        }
      }
      ...
      const effectiveToolKey =
        useCompletePreference && (toolKey === "jee-main" || toolKey === "csab")
      ...
      if (response.rankLocked && !isDevelopmentMode) {
        if (
          isValidRank(formData.crlRank) ||
          (isIIT && isValidRank(formData.crlRank))
        ) {
          setRankLocked(true);
        }
      ...
      if (prefill) {
        setFormData((prev) =>
          mergePrefillIntoForm(prev, prefill, false, isJACDelhi, isIIT),
        );
      ...
      if (code === "LIMIT_EXCEEDED") {
        toast.error(
          "Your limit has been exceeded! Please contact to your alloted mentor",
        );
        ...
        toast.error(
          "Your plan doesn't include access to this type of choice filling (IIT/JEE Main). Please contact support.",
        );
        ...
        toast.error(
          "You don't have an active counselling plan. Please purchase one to use this tool.",
        );
        ...
        toast.error(
          error.message || "Please login to use the Choice Filling tool.",
        );
        router.push("/auth");
      } else {
        toast.error(
          error.message || "Failed to generate choice list. Please try again.",
        );
      }
    } finally {
      setLoading(false);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8 items-start">
        ...
        <div className="flex flex-col space-y-3 sm:space-y-6">
          ...
          <div className="p-3 sm:p-6 bg-[var(--background)] border border-[var(--border)] rounded-lg sm:rounded-xl shadow-sm">
            ...
            <p className="text-xs sm:text-sm text-[var(--muted-text)] mt-1">
              Personalized, rank-optimized list ready for{" "}
              {isUPTAC || productSlug === "uptac"
                ? "UPTAC/AKTU/UPTU"
                : isCSAB || productSlug === "csab"
                  ? "CSAB Special Round"
                  : "JoSAA"}
            </p>
          </div>
        ...
        <div className="bg-[var(--background)] border border-[var(--border)] rounded-lg sm:rounded-xl shadow-lg p-3 sm:p-6 md:p-8 relative">
            ...
            <div className="absolute inset-0 z-10 rounded-lg sm:rounded-xl bg-white/80 backdrop-blur-[3px] flex flex-col items-center justify-center gap-4 border-2 border-orange-200">
              ...
              <div className="text-center px-6 max-w-md">
                ...
                <p className="text-sm text-gray-600 mt-2 leading-relaxed">
                  Choice filling is locked until your mentor completes your
                  assigned task. Contact your mentor or ask them to force-enable
                  access.
                </p>
              </div>
            </div>
          ...
          <form className="space-y-3 sm:space-y-5" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              >
                ...
                {isCrlRankRequired ? (
                  ...
                  <span className="text-[var(--muted-text)] font-normal text-[10px] ml-1">
                    (Optional)
                  </span>
                )}
              </label>
            ...
            <div>
              ...
              {categoryRankLocked && (
                <p className="text-xs text-amber-700 mt-1.5 font-medium">
                  Category Rank is pre-filled from your mentorship form and
                  cannot be changed.
                </p>
              )}
            </div>
            ...
            <div>
              ...
              >
                {isUPTAC && <option value="">Select Category</option>}
                {(
                  metadata?.categories || [
                    "OPEN",
                    "EWS",
                  ...
                  .filter(
                    (cat) =>
                      ((isJACDelhi && cat !== "GEN") || !isJACDelhi) &&
                      !(formData.gender === "Male" && isGirlRelated(cat)),
                  )
                  .map((cat) => (
                        ...
                        {metadata.iitList
                          ...
                          .map((iit) => (
                            ...
                            >
                              ...
                              {formData.includedIITs.includes(
                                iit.shortName,
                              ) && (
                                <svg
              ...
              <>
                ...
                <div>
                  ...
                  >
                    ...
                    {(metadata?.regions || ["Delhi", "Outside Delhi"]).map(
                      (region) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    className={`w-full p-2 sm:p-3 text-sm sm:text-base border rounded-lg shadow-sm bg-white outline-none transition ${
                      formData.subCategory.toLowerCase().includes("girl") &&
                      formData.gender === "Male"
                  ...
                  >
                    {(
                      ...
                      .filter(
                        (sub) =>
                          !(formData.gender === "Male" && isGirlRelated(sub)),
                      )
                      .map((sub) => (
                        <option
                          key={sub}
                ...
                <div>
                  ...
                  <div className="border border-[var(--border)] rounded-lg bg-white overflow-hidden shadow-sm">
                    ...
                    <div className="max-h-48 overflow-y-auto p-1.5">
                      {(() => {
                        const rawInstitutes = metadata?.institutes || [
                          "DTU",
                          "NSUT",
                          "IIITD",
                          "IGDTUW",
                          ...
                          <>
                            ...
                            {displayedInstitutes
                              .filter((inst) =>
                                inst
                                  .toLowerCase()
                                  .includes(jacInstituteSearch.toLowerCase()),
                              )
                              .map((inst) => (
                                <label
                                  key={inst}
                                ...
                                >
                                  <input
                                    type="checkbox"
                                    checked={formData.includedInstitutes.includes(
                                      inst,
                                    )}
                                  ...
                                  {formData.includedInstitutes.includes(
                                    inst,
                                  ) && (
                                    <svg
                                      className="w-4 h-4 text-[var(--primary)] flex-shrink-0"
                  ...
                  {formData.includedInstitutes.length > 0 && (
                    <p className="text-[10px] sm:text-xs text-[var(--primary)] mt-1.5 ml-1 font-semibold">
                      ...
                      {formData.includedInstitutes.length > 1 ? "s" : ""}{" "}
                      selected
                    </p>
                  )}
                </div>
                        ...
                        {availableInstituteStates
                          ...
                          .map((state) => (
                            ...
                            >
                              <input
                                type="checkbox"
                                checked={formData.includedStates.includes(
                                  state,
                                )}
                                onChange={() =>
                                  handleIncludedStateToggle(state)
                                }
                                className="mr-3 w-4 h-4 accent-[var(--primary)] rounded border-[var(--border)]"
                              />
                              <span className="text-xs sm:text-sm flex-1 text-[var(--muted-text)] font-medium group-hover:text-[var(--foreground)]">
                    ...
                    <div className="max-h-48 overflow-y-auto p-1.5">
                      <label className="flex items-center p-2.5 hover:bg-[var(--muted-background)] rounded-md cursor-pointer transition-colors mb-1 group">
                        <input
                          ...
                          onChange={() => {
                            const all = metadata?.instituteTypes || [
                              "NIT",
                              "IIIT",
                              "GFTI",
                            ];
                    ...
                    <>
                      ...
                      {availableBranchGroups
                        .filter((group) =>
                          group
                            .toLowerCase()
                            .includes(branchGroupSearch.toLowerCase()),
                        )
                        .map((group) => (
                          <label
                            key={group}
                ...
                <div className="pt-2">
                  <label className="flex items-center gap-2.5 px-3 py-2.5 border border-[var(--border)] rounded-lg text-xs sm:text-sm font-medium transition cursor-pointer hover:bg-[var(--muted-background)]/50 bg-white">
                    ...
                    <span className="text-[var(--foreground)] select-none">
                      Generate / Export choice list as student (Removes
                      counsellor columns)
                    </span>
                  </label>
                </div>
                ...
                <div className="pt-2">
                  <label className="flex items-center gap-2.5 px-3 py-2.5 border border-[var(--border)] rounded-lg text-xs sm:text-sm font-medium transition cursor-pointer hover:bg-[var(--muted-background)]/50 bg-white">
                    <input
                      ...
                      onChange={(e) =>
                        setUseCompletePreference(e.target.checked)
                      }
                      className="w-4 h-4 accent-[var(--primary)] rounded border-[var(--border)] cursor-pointer"
                    />
                    <span className="text-[var(--foreground)] select-none">
                      Generate Complete Exhaustive List (Counsellor Mode -
                      bypasses rank limits)
                    </span>
                  </label>
                </div>
              )}
